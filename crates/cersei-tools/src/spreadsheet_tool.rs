...
async fn execute_info(req: SpreadSheetRequest, ctx: &ToolContext) -> ToolResult {
    let path = match resolve_path(ctx, &req.file_path) {
        Ok(p) => p,
    ...
    let sheets = match collect_sheet_summaries(workbook, req.include_ranges.unwrap_or(false)) {
        Ok(s) => s,
        Err(e) => return ToolResult::error(e),
    };
    ...
    for s in &sheets {
        out.push_str(&format!(
            "- {}: {}x{}, {} cells",
            s.name, s.total_rows, s.total_columns, s.non_empty_cells
        ));
        if let Some(r) = &s.used_range {
            out.push_str(&format!(", range {}", r));
        }
        out.push('\n');
    }
    ToolResult::success(out)
        .with_metadata(serde_json::json!({ "file_path": path, "sheets": sheets }))
}
...
async fn execute_read(req: SpreadSheetRequest, ctx: &ToolContext) -> ToolResult {
    ...
    if sheet_names.is_empty() {
        return ToolResult::error("No sheets found");
    }
    let name = req
        .sheet_name
        .clone()
        .unwrap_or_else(|| sheet_names[0].clone());
    let range = match worksheet_range(&mut workbook, &name) {
        Ok(r) => r,
        Err(e) => return ToolResult::error(e),
    };
    ...
    let clim = req
        .col_limit
        .unwrap_or(MAX_COLUMN_LIMIT)
        .min(MAX_COLUMN_LIMIT);
    let sel = match build_selection(
        &range,
        req.range.as_deref(),
        req.start_row,
        req.start_col,
    ...
    let rows: Vec<Vec<String>> = range
        ...
        .map(|r| {
            r.iter()
                .skip(sel.start_col.saturating_sub(sheet_start.1 as usize))
                .take(sel.end_col.saturating_sub(sel.start_col))
                .map(|c| c.to_string())
                .collect()
        })
        .collect();
    ...
    ToolResult::success(out)
        .with_metadata(serde_json::json!({ "file_path": path, "sheet": name, "rows": rows.len() }))
}
...
fn collect_sheet_summaries(
    ...
    for n in names {
        ...
        let ur = if ranges && r.width() > 0 && r.height() > 0 {
            Some(format!(
                "{}{}:{}{}",
                col_label(start.1 as usize),
                start.0 + 1,
                col_label(start.1 as usize + r.width() - 1),
                start.0 as usize + r.height()
            ))
        } else {
            None
        };
        let cells = r
            .rows()
            .map(|row| row.iter().filter(|c| !c.to_string().is_empty()).count())
            .sum();
        sheets.push(SheetSummary {
            name: n,
            total_rows: r.height(),
            total_columns: r.width(),
...
fn build_selection(
    ...
    if let Some(r) = req_range {
        ...
        b.start_row = b.start_row.max(s_r);
        b.start_col = b.start_col.max(s_c);
        b.end_row = b.end_row.min(s_r + t_r);
        b.end_col = b.end_col.min(s_c + t_c);
        let u_r = b.end_row;
        let u_c = b.end_col;
        b.end_row = b.start_row.saturating_add(r_lim).min(b.end_row);
        b.end_col = b.start_col.saturating_add(c_lim).min(b.end_col);
        b.truncated_rows = b.end_row < u_r;
...
fn parse_a1_cell(cell: &str) -> StringResult<(usize, usize)> {
    ...
    for c in t.chars() {
        ...
        if c.is_ascii_alphabetic() && ds.is_empty() {
            ls.push(c.to_ascii_uppercase());
        } else if c.is_ascii_digit() {
            ds.push(c);
        } else {
            return Err(format!("Invalid cell {}", t));
        }
    }
    if ls.is_empty() || ds.is_empty() {
        return Err(format!("Invalid cell {}", t));
    }
    let mut col = 0usize;
    for c in ls.chars() {
        col = col * 26 + (c as u8 - b'A') as usize + 1;
    }
...
mod tests {
    ...
    async fn test_spreadsheet_info() {
        ...
        let res = tool
            .execute(
                json!({ "action": "info", "file_path": "tests/fixtures/Spreadsheet.xlsx" }),
                &test_ctx(),
            )
            .await;
        assert!(!res.is_error);
