...
mod tests {
    ...
    async fn test_store_and_recall_memory_fallback() {
        ...
        let store_res = store.execute(json!({
            "content": "The database port is 5433.",
            "memory_type": "project"
        }), &ctx).await;
        let store_out = store_res.content;
        assert!(store_out.contains("might be disabled or unavailable"), "got: {}", store_out);

        // 2. Recall works with graceful message or empty
        let recall_res = recall.execute(json!({
            "query": "database port",
            "limit": 5
        }), &ctx).await;
        assert!(recall_res.content.contains("No matching memories"), "got: {}", recall_res.content);
    }
}
