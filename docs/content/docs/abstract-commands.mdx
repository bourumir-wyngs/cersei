...
|----------|-------------|
| `Ctrl+F` | Add an instruction while the agent is running |
...
| `/reviewer <name>` | | List reviewer models or switch the reviewer model |
| `/tools` | | List model-visible tools for coding and reviewer agents |
| `/effort <value>` | | Show or set thinking tokens (`low`, `medium`, `high`, `max`, or a number) |
