...
description: Tool trait, 35 built-in tools, permission system, bash classifier, skill discovery.
---
...
Tool execution system with 35 built-in tools, a granular permission model, a bash command safety classifier, and skill discovery from Claude Code and OpenCode formats.
...
|----------|-------|----------------|
| `cersei::tools::all()` | 35 | Everything below |
| `cersei::tools::coding()` | 10 | filesystem + shell + web |
| `cersei::tools::filesystem()` | 7 | Read, Write, Edit, Patch, Glob, Grep, NotebookEdit |
...
`SedTool` uses the real GNU `sed` binary from the host OS, not a Rust reimplementation. The tool always executes sed as `sed -E --sandbox`, so standard GNU sed addresses, ranges, substitutions, groups, and commands work, while sed's own file-access commands are blocked.

- Pass only the sed program in `script`.
- Do not include `sed`, `-i`, extra filenames, shell pipelines, or redirection.
- The tool picks the file from `file_path`, captures sed's stdout, writes that back to the file, and returns a unified diff.
- `quiet=true` is GNU sed `-n`. Use it only when your script explicitly prints the desired output with commands like `p`.
- `null_data=true` is GNU sed `-z` for NUL-delimited data.
- `e`, `r`, and `w` are rejected because `--sandbox` is enforced on every call.
...
|------|-----------|-------------|
| `XReadTool` | ReadOnly | Read the latest XFileStorage-backed file revision, optionally by tag range. |
| `XWriteTool` | Write | Replace a file through XFileStorage, assign fresh line tags, and flush to disk. |
| `XEditTool` | Write | Apply tag-addressed line edits to the latest XFileStorage revision. |
| `XRevertTool` | None | Restore the previous XFileStorage revision for a tracked file. |
| `GlobTool` | ReadOnly | Pattern matching across directories. Returns matching paths sorted by mtime. |
| `XGrepTool` | ReadOnly | Regex search over XFileStorage-backed file content, returning stable line tags. |
| `ListDirectoryTool` | ReadOnly | List directory contents with metadata, optionally recursively. |
| `NotebookEditTool` | Write | Edit Jupyter notebook cells by index. |
| `FileHistoryTool` | Write | Inspect retained XFileStorage revisions, diffs, and restores for tracked files. |
